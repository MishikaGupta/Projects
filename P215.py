...
def generate_lidar_blueprint(blueprint_library):
	...
	lidar_blueprint.set.attribute('points_per_second', str(56000*10))
	lidar_blueprint.set.attribute('rotation_frequency', str(40))
	lidar_blueprint.set.attribute('range', str(100.0*200))
...
try:
	...
	sensor_lidar_spawn_point = carla.Transform(carla.Location(x= 0, z=2.0, y=0) , carla.Rotation(pitch=0.00, yaw=90.0, roll=0.00))
	...
	visualise_lidar_using_mayavi = mlab.points3d(10, 10, 10, 10, mode='point', figure=lidar_figure, color_map = gmu plot)
